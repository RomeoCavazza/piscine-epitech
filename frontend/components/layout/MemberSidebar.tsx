...
import { getMemberAvatar } from "@/lib/avatar";
    ...
    <aside className="w-[260px] p-5 bg-[rgba(20,20,20,0.85)] backdrop-blur-[12px] flex flex-col border-l-2 border-[#4fdfff] rounded-xl shadow-[0_20px_60px_rgba(0,0,0,0.5)] animate-fade-in" style={{ animationDelay: "0.3s" }}>
      ...
      <div className="flex items-center justify-between mt-[15px] mb-[6px]">
        <h3 className="text-[12px] tracking-wider uppercase text-[#4fdfff] font-bold">
          MEMBRES {selectedServer && `(${members.length})`}
        </h3>
      ...
      {!selectedServer ? (
        <p className="text-[14px] text-white/40 italic">Sélectionnez un serveur</p>
      ) : members.length === 0 ? (
        <p className="text-[14px] text-white/40 italic">Aucun membre</p>
      ) : (
        <>
          {/* Owners */}
                ...
                .map((member) => (
                  ...
                  >
                    <div className="relative">
                      ...
                      />
                      <span className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 border-2 border-[rgba(20,20,20,0.85)] rounded-full ${getStatusColor(member.status)}`} />
                    </div>
